...
class FiveMinQuizReportScreen extends StatefulWidget {
  const FiveMinQuizReportScreen({super.key});

  @override
  State<FiveMinQuizReportScreen> createState() => _FiveMinQuizReportScreenState();
}

class _FiveMinQuizReportScreenState extends State<FiveMinQuizReportScreen> {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      final res = await ApiService.getExamReports(
        type: "QUIZ", // Filter by 5-Min Quizzes
  ...
  Future<void> _exportToExcel() async {
    ...
    Sheet sheetObject = excel["5-Min Quiz Report"];
    excel.delete('Sheet1');
    sheetObject.appendRow([
      TextCellValue('Student Name'), TextCellValue('Phone'), TextCellValue('Board'),
      TextCellValue('Standard'), TextCellValue('Medium'), TextCellValue('Quiz Title'),
      TextCellValue('Score'), TextCellValue('Total'), TextCellValue('Date'),
    ]);
    ...
    if (fileBytes != null) {
      ...
      final file = File('${directory.path}/FiveMin_Quiz_Report_$timestamp.xlsx');
        ...
        child: Row(
          children: [
            Container(
              ...
              child: Icon(Icons.timer_rounded, color: Colors.purple.shade700, size: 24),
            ),
  ...
  Widget _buildEmptyState() {
    return Center(child: Text("No quiz results found", style: GoogleFonts.poppins(color: Colors.grey)));
  }
}
