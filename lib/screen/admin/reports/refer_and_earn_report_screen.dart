...
class _ReferAndEarnReportScreenState extends State<ReferAndEarnReportScreen> {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      final res = await ApiService.getReferAndEarnReports(
  ...
  Future<void> _exportToExcel() async {
    ...
    Sheet sheetObject = excel["Refer and Earn"];
    excel.delete('Sheet1');

    sheetObject.appendRow([
      TextCellValue('Referrer Name'),
      TextCellValue('Referred Student'),
      TextCellValue('Date'),
      TextCellValue('Points Earned'),
    ]);

    for (var row in _allData) {
      sheetObject.appendRow([
        TextCellValue(row['referrer']?.toString() ?? ''),
        TextCellValue(row['referred']?.toString() ?? ''),
        TextCellValue(row['date']?.toString() ?? ''),
        IntCellValue(int.tryParse(row['points']?.toString() ?? '0') ?? 0),
      ]);
    }
    ...
    if (fileBytes != null) {
      ...
      final file = File('${directory.path}/Refer_And_Earn_Report_$timestamp.xlsx');
  ...
  Widget _buildReportCard(dynamic row) {
    if (_searchQuery.isNotEmpty) {
      final referrerStr = row['referrer'].toString().toLowerCase();
      final referredStr = row['referred'].toString().toLowerCase();
      if (!referrerStr.contains(_searchQuery.toLowerCase()) && !referredStr.contains(_searchQuery.toLowerCase())) {
            ...
            Row(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Text(
                    "+${row['points']} pts",
                    style: GoogleFonts.poppins(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.orange.shade900),
                  ),
                ),
              ],
            ),
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.stars_rounded, size: 64, color: Colors.grey[300]),
