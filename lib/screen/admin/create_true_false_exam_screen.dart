...
class CreateTrueFalseExamScreen extends StatefulWidget {
  final Map<String, dynamic>? testToEdit;
  const CreateTrueFalseExamScreen({super.key, this.testToEdit});

  @override
  State<CreateTrueFalseExamScreen> createState() => _CreateTrueFalseExamScreenState();
}

class _CreateTrueFalseExamScreenState extends State<CreateTrueFalseExamScreen> with SingleTickerProviderStateMixin {
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              try {
                final response = await ApiService.deleteTrueFalseExam(id);
        ...
        if (mounted) {
          setState(() {
            ...
            if (questions.isNotEmpty) {
              ...
              while(parsedQuestions.length < marks) {
                parsedQuestions.add({
                  "question": "",
                  "questionImage": null,
                  "type": "MCQ", 
    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        if (response.statusCode == 201 || response.statusCode == 200) {
          CustomToast.showSuccess(context, (_isEditing || _editingId != null) ? "Test Updated Successfully!" : "True/False Exam Created Successfully!");
          _fetchTests();
          if (_editingId != null) {
            _resetForm();
  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          title: Text(
            _isEditing ? "Edit True/False Exam" : "Manage True/False Exam",
                      ...
                      children: [
                         ...
                         if (!_isManualEntry) ...[
                           ...
                           const Text("The PDF should contain an 'Overview' section and 'True/False' questions.", style: TextStyle(color: Colors.grey, fontSize: 12)),
                            ...
                            ...List.generate(_questions.length, (index) => _buildQuestionBlock(index)),
                         ]
                      ],
                    ),
                  ),
        ...
        children: [
          Row(
            children: [
              ...
              const Spacer(),
              // Type Selector
              Text("True/False", style: GoogleFonts.poppins(fontSize: 13, color: Colors.grey.shade600)),
            ],
          ),
          const SizedBox(height: 12),
